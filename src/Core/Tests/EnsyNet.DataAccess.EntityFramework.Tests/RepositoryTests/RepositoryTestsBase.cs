...
public abstract class RepositoryTestsBase : IDisposable
{
    ...
    protected readonly IRepository<TestEntity>Repository;
    protected readonly IPartitionedRepository<PartitionedTestEntity> PartitionedRepository;

    protected RepositoryTestsBase()
    {
        DbContext = new(DatabaseConfiguration.Options);
        Repository = new TestRepository(DbContext);
        PartitionedRepository = new PartitionedTestRepository(DbContext);
    }
    ...
    protected readonly PartitionedTestEntity ValidPartitionedEntity = new()
    {
        Name = "Test",
        OrgId = Guid.NewGuid(),
        Value = 42,
    ...
    protected static void AssertEntity(PartitionedTestEntity actualEntity, PartitionedTestEntity expectedEntity, bool assertAutoGenerated = true)
    {
        actualEntity.Id.Should().NotBe(Guid.Empty);

        actualEntity.Name.Should().Be(expectedEntity.Name);
